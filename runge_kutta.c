...
// Solve an ODE's using Runge-Kutta 4
void ode_solve(double (*f)(double, double), double t0, double t1, double dt, double y0,
    ...
    for (int i = 0; i < N-1; i++) {
        ...
        y[i+1] = y[i] + CONST*(c1 + 2.0*c2 + 2.0*c3 + c4);
        t[i+1] = t[i]+dt;
    }
}

// Solve a system of n ODE's using Runge-Kutta 4
void ode_solve_n(void (*f)(double, double*, double*), 
    ...
    for (int i = 0; i < n-1; i++) {
        ...
        f(t[i]+dt, var4, c4);
        for (int j = 0; j < dim; j++) {
            y[i+1][j] = y[i][j] + CONST*dt*(c1[j] + 2.0*c2[j] + 2.0*c3[j]+ c4[j]);
        }
        t[i+1] = t[i]+dt;
    }
}

// Calculate one Runge-Kutta 4 step
void runge_kutta_nstep(void (*f)(double, double*, double*),
    ...
    f(t+dt, var4, c4);
    for (int j = 0; j < dim; j++) {
        y_new[j] = y[j] + CONST*dt*(c1[j] + 2.0*c2[j] + 2.0*c3[j]+ c4[j]);
    }
}
